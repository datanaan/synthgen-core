SynthGen Core 开发路线图 v1.1
文档版本：v1.1
上一版本：v1.0
修订性质：回应第一轮对抗性审查（7项攻击，全部接受）
日期：2026-05-10
...
| # | 审查攻击 | 修订内容 |
|---|---------|---------|
| 1 | v1→v2 路由器重构量被低估 | v2 新增"执行路由器重构"组件，估算+2周；导读修正"原封不动"表述 |
| 2 | EvidencePackage v1 字段残缺 | EvidencePackage 增加字段适用性声明（applicability）；v1 诚实标记不适用的字段 |
| 3 | tail_report 应在 v1，反例搜索无理论基础 | tail_report 移入 v1；反例搜索降级为 v4-research（研究性里程碑） |
| 4 | v1 未声明生成路径 | v1 明确走纯物理采样路径；新增物理引擎组件；删除后筛选基础版（v1 不需要） |
| 5 | 路线图缺偏差声明 | 新增"诚实声明"章节 |
| 6 | ORDER 需 v1 前置 | v1 Schema DDL 已包含 ORDER 声明（v1 不使用但预留） |
| 7 | SynthLang 定位不清 | 每个版本明确用户接口：Python SDK + REST API；SynthLang 为内部编译目标 |
...
| 版本 | 产品能力 | 核心新组件数 | 估算 |
|------|---------|------------|------|
| **v1** 最小可运行 | Schema定义→数据导入→纯物理采样→值域约束验证→证据包输出 | 9 | 6-7周 |
| **v2** 约束完整 | 三类约束全做+执行路由器(5路径)+后筛选+审计日志 | 8 | 7-8周 |
...
**产品故事**：定义数据域，导入真实数据，在约束范围内物理采样，获得物理合法的合成数据和诚实的证据包。

**用户接口**：Python SDK + REST API。SynthLang 为内部编译目标，v1 不直接对用户暴露。

**生成路径**：纯物理采样。值域约束直接作为采样边界，物理引擎在约束域内采样，无需后筛选，无需数据引擎训练。
...
| 5 | 物理引擎 v1(纯采样) | 在约束域内采样（均匀/高斯等基础分布）+种子控制+批量生成 | #2 Schema | 1周 |
| 6 | 值域约束验证器 | 逐行验证采样结果是否在值域内（纯物理路径下应100%通过，验证器作为安全网） | #5 物理引擎 | 0.5周 |
| 7 | tail_report v1 | 值域约束排除率+偏差声明（物理优先认识论偏差）+data_grade | #6 验证器 | 0.5周 |
| 8 | EvidencePackage 构建器 v1 | schema_hash/constraint_summary/exclusion_rate/data_grade/row_count/provenance基础/conservative_tail_report。**字段适用性标注** | #7 tail_report + #6 验证器 | 1周 |
...
| constraint_summary | always | ✅ 填充 | 仅值域约束 |
...
#4 基础存储 ← #3 数据导入 ←────────────┘
      ↓
#5 物理引擎v1(纯采样) → #6 值域验证器 → #7 tail_report v1
                                                ↓
...
**输出**：1000行合成数据 + EvidencePackage（含 tail_report、偏差声明、字段适用性标注）
...
**产品故事**：行间依赖、窗口聚合、退化路径——完整的三类约束体系。后筛选路径接入，执行路由器支持5条退化路径，每条路径有身份声明和审计记录。

**用户接口**：Python SDK + REST API。SynthLang 为内部编译目标。

**v2 相对 v1 的重构**：

v1 的生成流程是"物理引擎采样→值域验证"，是单一固定路径。v2 引入执行路由器后，生成流程变为"约束分类→路由决策→多路径执行"。这不是在 v1 旁边"加一个引擎"，而是**重构执行调度逻辑**。类比：自行车加引擎不是后座绑发动机，而是改车架、传动、制动——摩托车是新的整车，不是自行车+引擎。
...
| 14 | 后筛选完整版 | 排除率预估(直方图/采样+保守偏向)+超时截断+排除率实时监控+误差界联动表(0-30%/30-70%/70-90%) | #13 执行路由器 | 1周 |
| 15 | 哈希链审计日志 | 创世记录+写入验证(prev_hash一致性)+分叉检测+每日全链校验+WORM存储 | v1 #4 存储 | 1周 |
| 16 | DURING/WHEN语义 | 条件约束：DURING column=value + WHEN condition THEN constraint | #12 分类器 + 引擎 | 0.5周 |
| 17 | EvidencePackage v2 | 新增statistical_fidelity(数据驱动时填充)+constraint_type_breakdown(三类)+身份声明(生成器身份) | #13 路由器 + #14 后筛选 | 0.5周 |
...
**输出**：
- 三类约束过滤后的数据
- EvidencePackage v2（含统计签名、约束分类、身份声明）
- tail_report（含误差界）
- 审计日志（哈希链可验证）
...
- [ ] 审计日志哈希链完整，可验证无篡改
- [ ] DURING/WHEN条件约束正确生效
- [ ] EvidencePackage v2 填充 statistical_fidelity 和 constraint_type_breakdown
...
| 21 | 持续对齐(UPDATE MODEL) | 新数据纳入+漂移检测(auto)+SAVE AS新版本+身份延续+代偿收敛时限 | #18 版本链 + v2 #13 执行路由器 | 1.5周 |
| 22 | tail_report v2(增强) | 排除率与data_grade联动+双变量/自相关误差界+fidelity_mismatch标记+代偿模型状态 | v2 #14 后筛选完整版 + #21 持续对齐 | 1周 |
...
#21 持续对齐(UPDATE MODEL) ←─────────┘
      ↓
#22 tail_report v2 ← v2后筛选 + #21 持续对齐
#24 偏差报告 ← #19 GC + #18 版本链
```

### 可并行开发

- #22 tail_report v2 + #23 存储模型层：与版本链开发并行
...
- tail_report v2（含误差界、fidelity_mismatch、代偿模型状态）
...
- [ ] tail_report v2 排除率与data_grade联动，含代偿模型状态
- [ ] 偏差报告字段完整，证明链可重建
...
| 29 | **反例搜索(research)** | 研究性里程碑：探索约束域边界+不满足约束的区域可视化。**理论基础待补充**，交付取决于 v2/v3 阶段的理论和工程预研 | v2 #13 路由器 + #14 后筛选 | 1.5周 |
...
| tail_report | v1 #7(v1基础版) + v3 #22(v2增强版) |
...
**覆盖率**：24/24 = 100%（反例搜索标记为 research）
...
| 审计不可变 | v2 #15 | 哈希链+WORM |
| 可追溯性 | v1 #8 + v3 #18 | provenance基础(v1) + 模型版本链(v3) |

**覆盖率**：10/10 = 100%

**关键修正**：v1.0 中"物理优先认识论偏差"仅在 v3 tail_report 中体现，v1.1 修正为从 v1 起即通过 tail_report 声明。
