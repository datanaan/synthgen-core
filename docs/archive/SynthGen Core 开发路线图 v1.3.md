SynthGen Core 开发路线图 v1.3
文档版本：v1.3
上一版本：v1.2
修订性质：终审裁定合并——将脚手架工程建议书作为暗线嵌入路线图
日期：2026-05-10
...
文档结构：**明线**（功能组件）+ **暗线**（脚手架工程），双线并行，同版本交付。
...
| # | 来源 | 修订内容 |
|---|------|---------|
| 1 | v1.1→v1.2 | 物理引擎v1限定为仅矩形约束域（BETWEEN），复合约束域留v2；估算1周→1.5周 |
| 2 | v1.1→v1.2 | v1声明无审计不可变保证；EvidencePackage v1新增audit_immutability: not_applicable |
| 3 | v1.1→v1.2 | v3 #22修正述为"tail_report增强版"——呈现增强，非新增计算 |
| 4 | v1.1→v1.2 | v2新增#15b数据引擎v1(KDE简化版)，3周 |
| 5 | v1.1→v1.2 | v4 #29增加依赖"待测模型接入协议（需在v3阶段定义）" |
| 6 | v1.1→v1.2 | v3 #21依赖图补充v2数据引擎依赖 |
| 7 | 终审裁定 | 脚手架工程作为暗线嵌入每个版本，含脚手架验收标准 |
| 8 | 终审裁定 | v3验收标准增加"定义待测模型接入协议"（v4反例搜索前置） |
| 9 | 终审裁定 | v2风险表增加工程执行注记：早期迭代先测3条不依赖数据引擎的退化路径 |
| 10 | 终审裁定 | 错误注入从v1推迟到v2（v1路径单一，无复杂退化路径可注入） |
...
| 版本 | 产品能力 | 功能组件数 | 脚手架组件数 | 功能估算 | 含脚手架估算 |
|------|---------|-----------|-------------|---------|-------------|
| **v1** 最小可运行 | Schema→导入→纯物理采样(矩形域)→值域验证→证据包 | 9 | 5 | 7-8周 | 8-9周 |
| **v2** 约束完整 | 三类约束+数据引擎+后筛选+执行路由器+审计 | 9 | 4 | 10-11周 | 11-12周 |
| **v3** 时间智能 | 时间旅行+持续对齐+模型版本管理+tail_report增强 | 7 | 3 | 4-5周 | 5-6周 |
| **v4** 高级分析 | 多窗口+完备度+反例研究 | 6 | 2 | 4-5周 | 4-5周 |
...
| 版本 | 功能组件 | 脚手架组件 | 功能估算 | 含脚手架估算 |
|------|---------|-----------|---------|-------------|
| v1 | 9 | Explain最小 + Trace最小 + 可观测性最小 + 测试框架 + CI/CD | 7-8周 | 8-9周 |
| v2 | 9 | Explain增强 + Trace增强 + 可观测性增强 + 错误注入 + 测试增强 | 10-11周 | 11-12周 |
| v3 | 7 | Explain增强 + Trace增强 + 可观测性增强 + 错误注入增强 + 测试增强 | 4-5周 | 5-6周 |
| v4 | 6 | Explain增强 + Trace增强 | 4-5周 | 4-5周 |
...
- **SDK + API**：1人（应用层）
- **脚手架 & 基础设施**：1人（可与SDK开发者复用）
- **总团队**：4-6人

v1 最小可运行阶段，2-3人即可并行（Parser+存储→物理引擎+SDK+脚手架）。
v2 需要数据引擎专人，建议 4-5人。
